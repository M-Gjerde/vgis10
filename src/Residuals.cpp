...
PointFrameResidual::PointFrameResidual(PointHessian* point_, std::shared_ptr<VO::Frame> host_, std::shared_ptr<VO::Frame> target_) :
	point(point_),
	host(host_),
	target(target_)
{
...
double PointFrameResidual::linearize(CalibHessian *HCalib, const Tracker* tracker)
{
	...
	FrameFramePrecalc* precalc = &(tracker->info->targetPrecalc[target->id]);
	float energyLeft=0;
	...
	for(int idx=0;idx<patternNum;idx++)
	{
        ...
        Vec3f hitColor = (VO::getInterpolatedElement33(dIl, Ku, Kv, tracker->firstFrame->width));
