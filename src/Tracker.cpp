...
void Tracker::initializeFromInitializer() {
    ...
    // Add First Frame
    //energyFunctional->insertFrame(firstFrame);
    firstFrame->trackingID = 0;
    secondFrame->trackingID = 1;
    ...
    frameHessians.emplace_back(secondFrame);
    initialized = true;
...
void Tracker::makeKeyFrame(std::shared_ptr<VO::Frame> frame) {
    ...
    //if (frame->trackingID > 1)
    //    framePoses.emplace_back(frame.get()); // Create new pose from new frame. But skip initializer stage
    ...
    */
    frame->trackingID = frameHessians.size() - 1;
    ...
    for(auto& host : frameHessians)
    {
        ...
        cv::imshow("PointHessian: " + std::to_string(host->trackingID), img);
    }
     */
...
void Tracker::traceNewCoarse(std::shared_ptr<VO::Frame> frame){
    ...
    for(size_t i = 0; i < frameHessians.size(); ++i){
        ...
        Vec2f aff = AffLight::fromToVecExposure(frameHessians[i]->abExposure, frame->abExposure, frameHessians[i]->pose.aff_g2l(), frameHessians[frame->trackingID]->pose.aff_g2l()).cast<float>();
...
void Tracker::flagFramesForMarginalization(std::shared_ptr<VO::Frame> sharedPtr) {
    ...
    for(int i=0;i<(int)frameHessians.size();i++)
    {
        ...
        Vec2 refToFh=AffLight::fromToVecExposure(frameHessians.back()->abExposure, fh->abExposure
                                                 , frameHessians.back()->pose.aff_g2l(), frameHessians[i]->pose.aff_g2l());
...
//				immature_notReady_skipped++;
            ...
            if((u > 0 && v > 0 && u < calibration->wG[1] && v < calibration->hG[1]))
            {
                ...
                float dist = coarseDistanceMap.fwdWarpedIDDistFinal[u+calibration->wG[1]*v] + (ptp[0]-floorf((float)(ptp[0])));
...
void Tracker::setPrecalcValues()
{
    for(auto& fh : frameHessians)
    {
        ...
        for(unsigned int i=0;i<frameHessians.size();i++)
            fh->targetPrecalc[i].set(fh->pose, frameHessians[i]->pose, hCalib.fxl(), hCalib.fyl(), hCalib.cxl(), hCalib.cyl());
    }
                                    ...
                                    PointHessian *phOut)
{
    ...
    {
        lastEnergy += point->linearizeResidual(&hCalib, 1000, residuals + i,lastHdd, lastbd, currentIdepth, targetFrames[i]);
