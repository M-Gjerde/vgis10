...
std::shared_ptr<VO::Frame> VO::FrameClass::getNextFrame(const CameraCalibration *calibration) {
    ...
    calibration->undistort(frame, 1, 0, 1);
    // Make image pyramids
...
void VO::FrameClass::makeImagePyramid(Frame *frame, const CameraCalibration *calibration) {
    ...
    for (auto pix: frame->dataRawRectified) {
        frame->pyramidNoPhotometric[0].emplace_back(pix, pix, pix);
    ...
    for (int lvl = 0; lvl < calibration->pyrLevelsUsed; ++lvl) {
        ...
        frame->pyramidNoPhotometric[lvl].resize(height * width);

        if (lvl > 0) {
            int wlm1 = frame->widthLevel[lvl - 1];
            Eigen::Vector3f *dI_lm = frame->pyramidNoPhotometric[lvl - 1].data();
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {

                    frame->pyramidNoPhotometric[lvl][x + y * width][0] = 0.25f * (dI_lm[2 * x + 2 * y * wlm1][0] +
        ...
        for (int idx = width; idx < width * (height - 1); idx++) {
            float dx = 0.5f * (frame->pyramidNoPhotometric[lvl][idx + 1][0] - frame->pyramidNoPhotometric[lvl][idx - 1][0]);
            float dy = 0.5f * (frame->pyramidNoPhotometric[lvl][idx + width][0] - frame->pyramidNoPhotometric[lvl][idx - width][0]);
            if (!std::isfinite(dx)) dx = 0;
            if (!std::isfinite(dy)) dy = 0;

            frame->pyramidNoPhotometric[lvl][idx][1] = dx;
            frame->pyramidNoPhotometric[lvl][idx][2] = dy;
            frame->absSquaredGrad[lvl][idx] = dx * dx + dy * dy;
            // With gamma calibration
            float color = (frame->pyramidNoPhotometric[lvl][idx][0]);
            int c = color + 0.5f;
            if (c < 5) c = 5;
            if (c > 250) c = 250;
