...
ImmaturePoint::ImmaturePoint(float uCoord, float vCoord, float t, const std::shared_ptr<VO::Frame>& frame) : u(uCoord), v(vCoord), type(t), host(frame.get())  {
...
ImmaturePointStatus ImmaturePoint::traceOn(std::shared_ptr<VO::Frame> frame, const Mat33f &hostToFrame_KRKi, const Vec3f &hostToFrame_Kt, const Vec2f &hostToFrame_affine, CalibHessian* HCalib, const CameraCalibration* calibration, bool debugPrint=false){
